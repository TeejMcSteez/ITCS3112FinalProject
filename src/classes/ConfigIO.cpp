...
/* TODO: There is a bug such that if there are improper amount of lines the program will crash with no output
...
std::vector<ModelParams> load_models_csv(const std::string & path) {
    ...
    while (std::getline(in, line)) {
        ...
        if (cells.size() < 5) throw std::runtime_error("Invalid config file");
        ModelParams m = ModelParams(cells[0], std::stoi(cells[1]), std::stoi(cells[2]), std::stoi(cells[3]), std::stoi(cells[4]));
